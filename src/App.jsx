...
const DEFAULT_MENTEES = [
  { id: 1, name: "Mentorado 1", slug: "mentorado-1" },
  { id: 2, name: "Mentorado 2", slug: "mentorado-2" },
  { id: 3, name: "Mentorado 3", slug: "mentorado-3" }
];
const THEMES = [
...
function EditableText({ value, onSave, style, multiline, fs }) {
...
const TarefaCard = memo(({ t, expanded, onToggleExpand, accent, bg, text, today, fs, onToggle, onDelete, onEditTitulo, onAddAcao, onToggleAcao, onDelAcao, onEditAcao }) => {
  const acoes = t.acoes || [];
  ...
  const mkC = (done, sq) => ({ width: sq ? 18 : 22, height: sq ? 18 : 22, minWidth: sq ? 18 : 22, borderRadius: sq ? 4 : "50%", border: `2px solid ${accent}`, background: done ? accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer" });

  return (
    <div style={{ background: `${accent}0d`, border: `1px solid ${accent}22`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div style={mkC(t.done, false)} onClick={() => onToggle(t.id)}>
          {t.done && <span style={{ color: bg, fontSize: fs.xs }}>✓</span>}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            <div style={{ flex: 1, fontSize: fs.base, fontWeight: "bold", lineHeight: 1.4, textDecoration: t.done ? "line-through" : "none", opacity: t.done ? 0.4 : 1 }}>
              <EditableText value={t.titulo} onSave={(v) => onEditTitulo(t.id, v)} fs={fs} style={{ background: `${accent}0e`, border: `1px solid ${accent}28`, borderRadius: 6, padding: "4px 8px", color: text, fontSize: fs.base, fontFamily: "'Palatino Linotype',serif", outline: "none" }} />
            </div>
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
              ...
              <button onClick={() => onDelete(t.id)} style={{ background: "none", border: "none", color: accent, opacity: 0.3, cursor: "pointer", fontSize: 22, padding: "0 2px", lineHeight: 1 }}>×</button>
            </div>
          </div>
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${accent}18` }}>
          ...
          {acoes.map(a => (
            <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={mkC(a.done, true)} onClick={() => onToggleAcao(t.id, a.id)}>
                {a.done && <span style={{ color: bg, fontSize: fs.xs - 1 }}>✓</span>}
              </div>
              <span style={{ flex: 1, fontSize: fs.sm, textDecoration: a.done ? "line-through" : "none", opacity: a.done ? 0.4 : 0.9, color: text }}>
                <EditableText value={a.text} onSave={(v) => onEditAcao(t.id, a.id, v)} fs={fs} style={{ background: `${accent}0e`, border: `1px solid ${accent}28`, borderRadius: 6, padding: "3px 8px", color: text, fontSize: fs.sm, fontFamily: "'Palatino Linotype',serif", outline: "none" }} />
              </span>
              <button onClick={() => onDelAcao(t.id, a.id)} style={{ background: "none", border: "none", color: accent, opacity: 0.3, cursor: "pointer", fontSize: 18, lineHeight: 1 }}>×</button>
            </div>
          ))}
...
export default function App() {
  ...
  const mkCircle = (done) => ({ width: 22, height: 22, minWidth: 22, borderRadius: "50%", border: `2px solid ${accent}`, background: done ? accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2, flexShrink: 0 });
  const empty = { textAlign: "center", opacity: 0.3, fontSize: fs.sm, fontStyle: "italic", padding: "28px 0" };
  const flatCard = { background: `${accent}0d`, border: `1px solid ${accent}22`, borderRadius: 12, padding: isDesktop ? "16px 18px" : "13px 14px", marginBottom: 10, display: "flex", alignItems: "flex-start", gap: 12 };

  const cardProps = { accent, bg, text, today, fs, onToggle: toggleTarefa, onDelete: delTarefa, onEditTitulo: editTitulo, onAddAcao: addAcao, onToggleAcao: toggleAcao, onDelAcao: delAcao, onEditAcao: editAcao, onToggleExpand: toggleExpand };
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {mentees.map(m => (
          <button key={m.id} onClick={() => setMenteeId(m.id)} style={{
            ...
            border: "none", borderRadius: 20, padding: "8px 16px", fontSize: fs.sm, cursor: "pointer",
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#12121f", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Palatino Linotype',serif", color: "#f0ece0" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, letterSpacing: 3, textTransform: "uppercase", color: "#e2b96f", opacity: 0.7 }}>Carregando...</div>
      </div>
    </div>
  );
  ...
  if (!role) return (
    <div style={{ minHeight: "100vh", background: bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, fontFamily: "'Palatino Linotype',serif" }}>
      <div style={{ textAlign: "center", marginBottom: 44, opacity: ready ? 1 : 0, transform: ready ? "translateY(0)" : "translateY(16px)", transition: "all 0.5s ease" }}>
        {logoUrl ? <img src={logoUrl} alt="logo" style={{ width: 72, height: 72, borderRadius: 14, objectFit: "cover", marginBottom: 16 }} /> : <div style={{ fontSize: 48, marginBottom: 14 }}>🧠</div>}
    ...
    return (
      <div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ height: 6, background: `${accent}18`, borderRadius: 3, marginBottom: 6, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${pct}%`, background: accent, borderRadius: 3, transition: "width 0.6s" }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: fs.sm, opacity: 0.5 }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
          {[{ l: "Metas", t: metas.length, d: done }, { l: "Hoje", t: hoje.length, d: hojeDone }].map(s => (
            <div key={s.l} style={{ flex: 1, background: `${accent}0e`, borderRadius: 12, padding: "16px 18px", border: `1px solid ${accent}1a` }}>
              <div style={{ fontSize: fs.label, letterSpacing: 2, textTransform: "uppercase", color: text, opacity: 0.45 }}>{s.l}</div>
              <div style={{ fontSize: fs.xl, color: accent, margin: "6px 0 2px" }}>{s.d}</div>
              <div style={{ fontSize: fs.xs, color: text, opacity: 0.4 }}>de {s.t}</div>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        {metas.map(m => (
          <div key={m.id} style={flatCard}>
            <div style={{ ...mkCircle(m.done), cursor: "pointer" }} onClick={() => toggleMeta(m.id)}>{m.done && <span style={{ color: bg, fontSize: fs.xs }}>✓</span>}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: fs.base, lineHeight: 1.5, textDecoration: m.done ? "line-through" : "none", opacity: m.done ? 0.4 : 1 }}>
                <EditableText value={m.text} onSave={(v) => editMeta(m.id, v)} fs={fs} style={{ background: `${accent}0e`, border: `1px solid ${accent}28`, borderRadius: 6, padding: "4px 8px", color: text, fontSize: fs.base, fontFamily: "'Palatino Linotype',serif", outline: "none" }} multiline />
              </div>
              <div style={{ fontSize: fs.xs, color: accent, opacity: 0.55, marginTop: 3 }}>{m.done ? `✓ ${m.doneAt}` : `Criada ${m.createdAt}`}</div>
            </div>
            <button style={{ background: "none", border: "none", color: accent, opacity: 0.35, cursor: "pointer", fontSize: 22, padding: "0 2px", flexShrink: 0 }} onClick={() => delMeta(m.id)}>×</button>
          </div>
        ))}
      ...
      <div>
        ...
        {dates.map(d => (
          <div key={d} style={{ marginBottom: 20 }}>
            <div style={{ fontSize: fs.xs, color: accent, letterSpacing: 2, marginBottom: 8, opacity: 0.6 }}>{d === today ? "📅 Hoje" : d}</div>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
          {[{ l: "Metas", total: metas.length, done: md.length }, { l: "Tarefas", total: tarefas.length, done: td.length }].map(s => (
            <div key={s.l} style={{ flex: 1, background: `${accent}0e`, borderRadius: 12, padding: "16px 18px", border: `1px solid ${accent}1a` }}>
              <div style={{ fontSize: fs.label, letterSpacing: 2, textTransform: "uppercase", color: text, opacity: 0.45 }}>{s.l}</div>
              <div style={{ fontSize: fs.xl, color: accent, margin: "6px 0 2px" }}>{s.done}</div>
        ...
        {md.length > 0 && <><div style={secLabel}>Metas Concluídas</div>{md.map(m => (
          <div key={m.id} style={{ ...flatCard, opacity: 0.65 }}>
            <div style={mkCircle(true)}><span style={{ color: bg, fontSize: fs.xs }}>✓</span></div>
            <div style={{ flex: 1 }}>
    ...
    return (
      <div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: fs.sm, color: text, opacity: 0.5, marginBottom: 8 }}>Logo</div>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            {lc.logoUrl ? <img src={lc.logoUrl} alt="logo" style={{ width: 56, height: 56, borderRadius: 10, objectFit: "cover" }} /> : <div style={{ width: 56, height: 56, borderRadius: 10, background: `${accent}18`, border: `1px solid ${accent}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>🧠</div>}
            <div>
              ...
              <button onClick={() => lRef.current.click()} style={{ ...ghostBtn, width: "auto", padding: "8px 16px", marginBottom: 0 }}>📁 Escolher imagem</button>
        ...
        <div style={{ fontSize: fs.sm, color: text, opacity: 0.5, marginBottom: 5 }}>Nome principal</div>
        <RenameInput initial={lc.brandName} onSave={v => setLc(p => ({ ...p, brandName: v }))} style={inp} />
        <div style={{ fontSize: fs.sm, color: text, opacity: 0.5, marginBottom: 5 }}>Subtítulo</div>
        <RenameInput initial={lc.brandSub} onSave={v => setLc(p => ({ ...p, brandSub: v }))} style={inp} />
        <div style={{ fontSize: fs.sm, color: text, opacity: 0.5, marginBottom: 10, marginTop: 4 }}>Temas prontos</div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
          {THEMES.map(t => (
            <div key={t.label} onClick={() => setLc(p => ({ ...p, accentColor: t.accent, bgColor: t.bg }))}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, cursor: "pointer" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: t.accent, border: lc.accentColor === t.accent ? `3px solid ${text}` : "3px solid transparent" }} />
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="color" value={lc.accentColor} onChange={e => setLc(p => ({ ...p, accentColor: e.target.value }))} style={{ width: 40, height: 40, border: "none", borderRadius: 6, cursor: "pointer", padding: 0 }} />
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="color" value={lc.bgColor} onChange={e => setLc(p => ({ ...p, bgColor: e.target.value }))} style={{ width: 40, height: 40, border: "none", borderRadius: 6, cursor: "pointer", padding: 0 }} />
        ...
        <div style={{ borderTop: `1px solid ${accent}18`, paddingTop: 20, marginTop: 8 }}>
          ...
          {mentees.map(m => (
            <div key={m.id} style={{ marginBottom: 14, background: `${accent}08`, borderRadius: 10, padding: "14px 16px", border: `1px solid ${accent}18` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <RenameInput initial={m.name} onSave={name => renameMentee(m.id, name)} style={{ ...inp, marginBottom: 0, flex: 1 }} />
                <button onClick={() => delMentee(m.id)} style={{ background: "none", border: "none", color: accent, opacity: 0.4, cursor: "pointer", fontSize: 24, padding: "0 4px", flexShrink: 0 }}>×</button>
              </div>
              <div style={{ fontSize: fs.xs, color: accent, opacity: 0.6, marginBottom: 5 }}>Link de acesso:</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ fontSize: fs.xs, color: text, opacity: 0.7, background: `${accent}10`, padding: "7px 10px", borderRadius: 6, flex: 1, wordBreak: "break-all" }}>
                  {baseUrl}/{m.slug || toSlug(m.name)}
                </div>
                <button onClick={() => navigator.clipboard.writeText(`${baseUrl}/${m.slug || toSlug(m.name)}`)}
                  style={{ background: accent, color: bg, border: "none", borderRadius: 6, padding: "7px 12px", fontSize: fs.xs, cursor: "pointer", flexShrink: 0 }}>
                  Copiar
                </button>
              </div>
            </div>
  ...
  if (isDesktop) return (
    <div style={{ display: "flex", height: "100vh", background: bg, color: text, fontFamily: "'Palatino Linotype','Book Antiqua',Palatino,serif", overflow: "hidden" }}>
      <div style={{ width: 240, background: `${accent}08`, borderRight: `1px solid ${accent}18`, display: "flex", flexDirection: "column", padding: "28px 0 20px", flexShrink: 0 }}>
        <div style={{ padding: "0 20px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            {logoUrl ? <img src={logoUrl} alt="logo" style={{ width: 40, height: 40, borderRadius: 10, objectFit: "cover" }} /> : <div style={{ width: 40, height: 40, borderRadius: 10, background: `${accent}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>🧠</div>}
            <div>
          ...
          {role === "mentor" && (
            <div>
              ...
              {mentees.map(m => (
                <button key={m.id} onClick={() => setMenteeId(m.id)} style={{
                  display: "block", width: "100%", textAlign: "left", background: menteeId === m.id ? `${accent}22` : "transparent",
                  color: menteeId === m.id ? accent : text, border: "none", borderRadius: 6, padding: "6px 10px",
        ...
        <div style={{ flex: 1, padding: "0 12px" }}>
          {navItems.map(n => (
            <button key={n.id} onClick={() => setView(n.id)} style={{
              ...
              padding: "11px 16px", fontSize: fs.sm, cursor: "pointer",
        ...
        <div style={{ padding: "0 12px" }}>
          <button onClick={handleLogout} style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: `1px solid ${accent}28`, color: accent, borderRadius: 8, padding: "9px 16px", fontSize: fs.xs, cursor: "pointer", opacity: 0.6 }}>← Início</button>
        </div>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "24px 40px 16px", borderBottom: `1px solid ${accent}18` }}>
          <h1 style={{ fontSize: fs.lg, fontWeight: "normal", fontStyle: "italic", margin: 0, color: text }}>{titles[view]}</h1>
        </div>
  ...
  return (
    <div style={{ height: "100vh", background: bg, color: text, fontFamily: "'Palatino Linotype','Book Antiqua',Palatino,serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "14px 16px 10px", borderBottom: `1px solid ${accent}22`, display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        {logoUrl ? <img src={logoUrl} alt="logo" style={{ width: 34, height: 34, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} /> : <div style={{ width: 34, height: 34, borderRadius: 8, background: `${accent}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>🧠</div>}
        <div style={{ flex: 1 }}>
        ...
        <button onClick={handleLogout} style={{ background: "none", border: `1px solid ${accent}28`, color: accent, borderRadius: 6, padding: "5px 10px", fontSize: fs.label, cursor: "pointer", opacity: 0.7 }}>← Início</button>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 8px" }}>
        <MainContent />
      </div>
      <div style={{ display: "flex", gap: 4, padding: "8px 10px 16px", borderTop: `1px solid ${accent}18`, flexShrink: 0 }}>
        {navItems.map(n => (
          <button key={n.id} onClick={() => setView(n.id)} style={{
